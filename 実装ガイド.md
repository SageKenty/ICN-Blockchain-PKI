...
## 初回
```
docker build -t cefore-base -f cefore-base.Dockerfile .
```
以上を使ってbaseイメージを作る。
## 古いイメージが残ってる
```
docker image rm cefore-base
```
## cefnetdが動かなくなる
...
1. docker-composeファイルを実行する
詳細を言うと、ceforeから直接ログを取るのではなく、tcpdumpで取得したpcapログをwiresharkで読み込ませると言う形のようだ。
Dockerのブリッジネットワークをtcpdumpするのだが、権限の問題があってかなかなかうまくいかず、書き込みもされていなかった。
そこで、ファイルシステムの形にext4とnfsの違いがあるみたいで、nfs上だとどうにもtcpdumpがうまくいかないようだ。
そこで
```
/var/tmp/PKI_local
```
の上にプロジェクトを移して行うか、ログだけそこに書くかを選ぶ必要がある。
現に現在、ariesの`/var/tmp/PKI_logs`にログ用の保管庫を設定している。
一旦これで試してみる。

tcpdump
```
sudo tcpdump -i br-xxxxxx -nn -s0 -w /var/tmp/pki_logs/pki_all.pcap
```
ファイルシステムを見る方法
```
df -T .
